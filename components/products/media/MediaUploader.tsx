...
export default function MediaUploader({ productId, colorId }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!url.trim()) return
    await upload.mutateAsync({ product_id: productId, url: url.trim(), alt_text: altText || undefined, media_type: 'IMAGE', color_id: colorId })
    setUrl(''); setAltText('')
  }
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            />
            <button type="submit" disabled={upload.isPending || !url.trim()} className="px-3 py-2 bg-[#c9a465] text-white text-sm hover:bg-[#b8934f] disabled:opacity-50 whitespace-nowrap">
